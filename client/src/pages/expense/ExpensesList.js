    ...
    <>
      {loading ? <LoadingComponent/> :
       appErr || serverErr ? 
         <ErrorDisplayMessage >
        {serverErr} {appErr}
        </ErrorDisplayMessage> : <section className="py-6">
        <div className="container-fluid">
          <div className="position-relative border rounded-2">
            ...
            <table className="table">
              ...
              <tbody>
                {loading? <h1>Loading</h1> : appErr || serverErr ? 
                <div>Err</div>:expensesList?.docs?.length <= 0 ?
                 <h1>No Expense Found</h1> : expensesList?.docs?.map(exp=>(
